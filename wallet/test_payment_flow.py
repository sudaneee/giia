...
from src.models import FeeComponent, FeeStructure, Payment, SchoolClass, Section, Session, Student, Term
...
class FeeServiceOutstandingTests(TestCase):
    ...
    def test_get_default_school_fee_outstanding_for_unpaid_student(self):
        result = fee_service.get_default_school_fee_outstanding(
    ...
    def test_get_default_school_fee_outstanding_deducts_prior_payment(self):
    ...
    def test_get_guardian_fee_summary_lists_linked_children_only(self):
        parent_account = make_parent_account()
        ParentStudentLink.objects.create(parent_account=parent_account, student=self.fixture['student'])

        other_student = Student.objects.create(admission_number='GIIA/9999', first_name='Not', last_name='Mine')

        summary = fee_service.get_guardian_fee_summary(
            parent_account, self.fixture['session'], self.fixture['term'],
        )
        self.assertEqual(len(summary), 1)
        self.assertEqual(summary[0]['student'], self.fixture['student'])
        self.assertEqual(summary[0]['result']['balance'], Decimal('50000.00'))
...
class PayFeesViewTests(TestCase):
    ...
    def test_outstanding_fees_shows_balance(self):
